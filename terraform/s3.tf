...
resource "aws_s3_object" "ingest_lambda_code_deployment" {
  ...
  source_hash = filemd5("${path.module}/../deployment_zips/deploy_ingestion_lambda.zip")
}
...
resource "aws_s3_object" "process_lambda_code_deployment" {
  ...
  source_hash = filemd5("${path.module}/../deployment_zips/deploy_process_lambda.zip")
}
...
resource "aws_s3_object" "load_lambda_code_deployment" {
  ...
  source_hash = filemd5("${path.module}/../deployment_zips/deploy_load_lambda.zip")
}
