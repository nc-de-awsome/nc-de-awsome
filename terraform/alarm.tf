...
# load lambda alarm
resource "aws_cloudwatch_log_group" "load_zone_log" {
  name = "/aws/lambda/${var.load_lambda_name}"
}

resource "aws_cloudwatch_log_metric_filter" "load_error" {
  name           = "LoadErrorFilter"
  pattern        = "LoadError"
  log_group_name = aws_cloudwatch_log_group.load_zone_log.name

  metric_transformation {
    name      = var.load_error_metric_name
    namespace = var.metric_namespace
    value     = "1"
  }
}

resource "aws_cloudwatch_metric_alarm" "alert_load_error" {
  alarm_name          = "LoadErrorAlarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = var.load_error_metric_name
  namespace           = var.metric_namespace
  period              = "60"
  statistic           = "Sum"
  threshold           = "1"
  actions_enabled     = "true"
  alarm_description   = "This metric checks for any issue with the load lambda"
  alarm_actions       = ["${aws_sns_topic.error_alerts.arn}"]
}
